...
GBCore.Functions.CreateCallback('gamblerx2-anticheat:server:GetPermissions', function(source, cb)
    local group = GBCore.Functions.GetPermission(source)
    cb(group)
end)

-- Execute ban --

RegisterServerEvent('gamblerx2-anticheat:server:banPlayer')
AddEventHandler('gamblerx2-anticheat:server:banPlayer', function(reason)
    local src = source
    GBCore.Functions.ExecuteSql(false, "INSERT INTO `bans` (`name`, `steam`, `license`, `discord`,`ip`, `reason`, `expire`, `bannedby`) VALUES ('"..GetPlayerName(src).."', '"..GetPlayerIdentifiers(src)[1].."', '"..GetPlayerIdentifiers(src)[2].."', '"..GetPlayerIdentifiers(src)[3].."', '"..GetPlayerIdentifiers(src)[4].."', '"..reason.."', 2145913200, '"..GetPlayerName(src).."')")
    DropPlayer(src, "Jus buvote aptiktas perdaug kartu nuo musu anticheato! Jei tai klaida susisiekite su mumis! https://dsc.gg/Gamblerv2")
    TriggerClientEvent('chatMessage', -1, "GAMBLER-APSAUGA", "warning", GetPlayerName(src).." buvo užblokuotas: " ..reason )
end)

-- Fake events --
function NonRegisteredEventCalled(CalledEvent, source)
    TriggerClientEvent("gamblerx2-anticheat:client:NonRegisteredEventCalled", source, "Cheating", CalledEvent)
end
...
GBCore.Functions.CreateCallback('gamblerx2-anticheat:server:HasWeaponInInventory', function(source, cb, WeaponInfo)
    local src = source
